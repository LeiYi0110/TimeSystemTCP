...
//				webChannelHandlerContext.writeAndFlush(msg);
	...
	private void photoFileUpload(ActionContext context, PhotoFileUpload photoFileUpload, Message msg, short seq, short number, boolean checkresult) {
		try {
			...
			webPhoto.setLic(loginInfo.getTrainingCar().getTrainingLicNum());
			...
			//更新图片信息
			/*
			UploadPhoto uploadPhoto = loginInfo.getUploadPhoto();
			PhotoInfo photoInfoByNum = uploadPhoto.getPhotoInfoByNum(photoFileUpload.getPhotoNum());
			if(photoInfoByNum!=null){
				photoInfoByNum.setUrl(photoUrl);
				uploadPhoto.setPhotoInfo(photoInfoByNum);
				TCPMap.getInstance().getLoginInfoMap().put(msg.getHeader().getMobile(), loginInfo);
			}*/
			
			//更新数据库图片信息
			deviceImageService.updateUrl(Integer.parseInt(photoFileUpload.getPhotoNum()),loginInfo.getDevice().getId(), photoUrl);
			List<DeviceImage> list = deviceImageService.findByNum(photoFileUpload.getPhotoNum(),loginInfo.getDevice().getId());
			...
			if(jttChannel != null && list != null && list.size() != 0){
				com.bjxc.model.TrainingCar jttTrainingCar = loginInfo.getJttTrainingCar();
	...
	private void photoUploadInit(ActionContext context, PhotoUploadInit photoUploadInit, Message msg, short seq, short number, boolean checkresult) {
		...
		if(!checkresult){
			...
			try {
				LoginInfo loginInfo = TCPMap.getInstance().getLoginInfoMap().get(msg.getHeader().getMobile());
				//照片编号重复检查
				List<DeviceImage> list = deviceImageService.findByNum(photoUploadInit.getPhotoNum(),loginInfo.getDevice().getId());
				if(list == null || list.size() == 0){
					data.setResultCode((byte)0);//0允许，255拒绝
					int objectId = 0;
					byte photoType = photoUploadInit.getPhotoType();
					if(photoType==20||photoType==21){	//学员或教练员ID，发起图片的事件类型为20,21的时候为教练员
						Coach coachByNum = coachService.getCoachByNum(photoUploadInit.getStuNum());
						if(coachByNum!=null){
							objectId = coachByNum.getId();
						}
					}else{
						Student studentByNum = studentService.getStudentByNum(photoUploadInit.getStuNum());
						if(studentByNum!=null){
							objectId = studentByNum.getId();
						}
					}
					...
					deviceImage.setDeviceId(loginInfo.getDevice().getId());
					deviceImage.setObjectId(objectId);
					deviceImage.setImageNum(photoUploadInit.getPhotoNum());
					deviceImage.setUploadType((int) photoUploadInit.getUpMode());
					deviceImage.setChannelNo((int) photoUploadInit.getCameraNum());
					int width =	0;
					int height = 0;
					switch (photoUploadInit.getPhotoSize()) {
						case 1:
							width = 240;
							height=320;
							break;
					...
					deviceImage.setImageWidth(width);
					deviceImage.setImageHeight(height);
					deviceImage.setEvent((int) photoUploadInit.getPhotoType());
					deviceImage.setLessonId(photoUploadInit.getClassId());
					...
					deviceImage.setCreatetime(dateFormatter.parse(photoUploadInit.getLocationInfo().getTime()));
					...
					if(!photoUploadInit.getStuNum().equals("0000000000000000")){
						...
						if(photoUploadInit.getPhotoType() != 17 && photoUploadInit.getPhotoType() != 18
							...
							deviceImage.setMinRecordId(studentLastLog.getId());
						}
...
//					data.setResultCode((byte)1);	//照片编号重复或错误
	...
	private void studyingTime(ActionContext context, StudyingTime studyingTime, Message msg, short seq, short number, boolean checkresult) {
		...
		try {
			...
			trainingLog.setLocationinfoid(locationInfoId.getId());
	...
	private void loginStudent(ActionContext context, LoginStudent loginStudent, Message msg, short seq, short number, boolean checkresult) {
		...
		try {
			if(!checkresult){
				...
				if(studentByNum==null){	//无效的学员编号
				...
				}else if(!coachByNum.getTeachpermitted().toUpperCase().contains(studentByNum.getTraintype().toUpperCase())){	//准教车型与培训车型不符
					...
					trainingRecord.setTrainingcarId(loginInfo.getTrainingCar().getTrainingCarId());
					...
					trainingRecord.setSubjectId(studentByNum.getSubjectId());
					Integer lastTrainingLogCode = trainingRecordService.getLastTrainingLogCode(studentByNum.getId());
					String trainingLogCode = new DecimalFormat("00000").format(lastTrainingLogCode+1);
					trainingRecord.setEtrainingLogCode(trainingLogCode);
					...
					ChannelHandlerContext jttChannel = TCPMap.getInstance().getChannelMap().get(JttUtils.JTT_CHANNEL);
					if(jttChannel != null){
						JttMessage jttMessage = JttUtils.coachTransform(studentByNum.getIdcard(),studentByNum.getName(), loginInfo.getJttTrainingCar());
						jttChannel.writeAndFlush(jttMessage);
					}
				}
			}
	...
	private void loginCoach(ActionContext context, LoginCoach loginCoach, Message msg, short seq, short number, boolean checkresult) {
		...
		try {
			if(!checkresult){
				...
				if(coachByNum==null){	//教练编号不存在
				...
				}else if(!coachByNum.getTeachpermitted().toUpperCase().contains(loginCoach.getTeachCarType())){	//车型不正确
					 logger.info("车型不正确");
					...
					ChannelHandlerContext jttChannel = TCPMap.getInstance().getChannelMap().get(JttUtils.JTT_CHANNEL);
					if(jttChannel != null){
						JttMessage jttMessage = JttUtils.coachTransform(coachByNum.getIdcard(),coachByNum.getName(), loginInfo.getJttTrainingCar());
						jttChannel.writeAndFlush(jttMessage);
					}
				}
			}
