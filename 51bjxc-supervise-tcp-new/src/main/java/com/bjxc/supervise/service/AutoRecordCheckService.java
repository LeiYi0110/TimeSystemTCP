...
public class AutoRecordCheckService {
	...
	public void checkMinRecord(TrainingLog tlog) {
		...
		//
   		LocationInfo locationInfo = locationInfoService.getLocationInfoById(tlog.getLocationid());
		...
		if (coachByNum.getIsProvince() != 1) {
 		...
 		if (!Utils.checkCarTrain(coachByNum.getTeachpermitted().toUpperCase(),studentByNum.getTraintype().toUpperCase())) {
...
//			continue;
//		}
//		if (!Utils.checkCarTrain(trainingLogWithLocation.getTrainingCarPerdritype().toUpperCase(),studentByNum.getTraintype().toUpperCase())) {
...
//		}
		...
		// 5:连续2分钟或以上分钟学时中最大速度为0或行驶记录速度为0，该2分钟或以上的分钟学时无效；（参数：最大允许停车时间，默认2分钟）（学时）
 		...
 		if (locationInfo.getCarSpeed() == 0) {
			...
			//判断前一个学时速度是否为0
			TrainingLog recordFirstLog = trainingLogService.getRecordFirstLog(tlog.getTrainingrecordid());
			if(recordFirstLog == null || recordFirstLog.getRecordnum().equals(tlog.getRecordnum())){
				//判断该学时是当前日志的第一个学时,不判断连续速度
			}else{
				...
				if( _trainingLog != null ){
					LocationInfo _locationInfo = locationInfoService.getLocationInfoById(_trainingLog.getLocationid());
		...
		// 学时自动审核通过
		trainingLogService.updatePass(tlog.getId(), Valid, "学时自动审核通过");
	...
	public void checkTrainingRecord(TrainingRecord teachLog) {
		...
		//更新速度条数
		trainingRecordService.updateSpeedCount(teachLog.getId());
	 ...
	 * @param teachLog
	 */
	private void checkPhoto(TrainingRecord teachLog) {
