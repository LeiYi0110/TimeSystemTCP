		...
		try{
			if(message.getHeader() != null && message.getHeader().getId() == 0x0102){
				...
				if(devAssignByMobile!=null){	//已注册的设备
					...
					com.bjxc.model.TrainingCar trainingCar = trainingCarService.getTrainingCarById(devAssignByMobile.getTrainingcarId());
...
//					boolean checkresult = true;
					...
					if(checkresult){
						...
						if(jttChannel != null){
							JttMessage jttMessage = JttUtils.carTransform(terminalDevice, trainingCar,insById);
