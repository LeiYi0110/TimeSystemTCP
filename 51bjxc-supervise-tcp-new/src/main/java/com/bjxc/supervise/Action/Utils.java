...
//		extendMessageBody.setCheck(HexUtils.HexStringToBinary(encodedEncryptedStr));
	 ...
	 * 判断教练与学员车型是否相符
	 * @param coachCarType
	 * @param studentCarType
	 * @return
	 */
	public static boolean checkCarTrain(String coachCarType, String studentCarType) {
		switch (coachCarType) {
			case "A1":
				String[] arr = {"A1","A3","B1","B2","C1","C2","C3","C4","M"};
				return Arrays.asList(arr).contains(studentCarType);
			case "A2":
				String[] arr2 = {"A2","B1","B2","C1","C2","C3","C4","M"};
				return Arrays.asList(arr2).contains(studentCarType);
			case "A3":
				String[] arr3 = {"A3","C1","C2","C3","C4"};
				return Arrays.asList(arr3).contains(studentCarType);
			case "B1":
				String[] arr4 = {"B1","C1","C2","C3","C4","M"};
				return Arrays.asList(arr4).contains(studentCarType);
			case "B2":
				String[] arr5 = {"B2","C1","C2","C3","C4","M"};
				return Arrays.asList(arr5).contains(studentCarType);
			case "C1":
				String[] arr6 = {"C1","C2","C3","C4"};
				return Arrays.asList(arr6).contains(studentCarType);
			case "C2":
				String[] arr7 = {"C2"};
				return Arrays.asList(arr7).contains(studentCarType);
			case "C3":
				String[] arr8 = {"C3","c4"};
				return Arrays.asList(arr8).contains(studentCarType);
			case "C4":
				String[] arr9 = {"C4"};
				return Arrays.asList(arr9).contains(studentCarType);
			case "D":
				String[] arr10 = {"D","E","F"};
				return Arrays.asList(arr10).contains(studentCarType);
			case "E":
				String[] arr11 = {"E","F"};
				return Arrays.asList(arr11).contains(studentCarType);
			case "F":
				String[] arr12 = {"F"};
				return Arrays.asList(arr12).contains(studentCarType);
			case "M":
				String[] arr13 = {"M"};
				return Arrays.asList(arr13).contains(studentCarType);
			case "N":
				String[] arr14 = {"N"};
				return Arrays.asList(arr14).contains(studentCarType);
			default:
				return false;
		}
	}
