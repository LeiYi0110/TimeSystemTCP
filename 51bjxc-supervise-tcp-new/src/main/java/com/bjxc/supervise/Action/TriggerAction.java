...
//		context.write(studentLogin);
		switch (messageId) {
			...
			case -2:{	//伪造学员学车
				...
				String licNum = "京K9887学";
				String trainingCarNum = "9860371922632426";
				int trainingCarId = 93;
				
				String mobile = "15898877651";
				
				Integer lastTrainingLogCode = trainingRecordService.getLastTrainingLogCode(studentNum);	//该学员最后一条电子教学日志的编号
				...
				if(fakeStudent.getCount()>0){	//只造当天相应分钟学时记录
					...
					addTrainingRecord(context, studentNum, coachNum, studentId, coachId, deviceId, deviceNum,
							trainingCarId,trainingCarNum, mobile, trainingLogCode, subjectId, new Date(),fakeStudent.getCount(),lastPhotoNum, device);
					...
					for(int i=1;i<=subjectCount2;i++){	//科目2，每天每次学车1小时，需要？次
						...
						addTrainingRecord(context, studentNum, coachNum, studentId, coachId, deviceId, deviceNum, trainingCarId,trainingCarNum
					...
					for(int i=1;i<=subjectCount3;i++){	//科目3，每天每次学车1小时，需要？次
						...
						addTrainingRecord(context, studentNum, coachNum, studentId, coachId, deviceId, deviceNum,
								trainingCarId,trainingCarNum, mobile, trainingLogCode, subjectId, startTime,60,lastPhotoNum,device);
					}
				}
			}
...
//				context = TCPMap.getInstance().getChannelMap().get(packageMessage.getHeader().getMobile());
	...
	public static void addTrainingRecord(ChannelHandlerContext context, String studentNum, String coachNum, int studentId,
			int coachId, int deviceId, String deviceNum, int trainingCarId,String trainingcarNum, String mobile, int trainingLogCode, int subjectId,
		...
		//1、添加电子教学日志到数据库
		Integer trainingRecordId = FakeMethod.addTrainingRecord(studentNum,studentId, coachId, trainingCarId, deviceId, subjectId, trainingLogCode, startTime, endTime, deviceNum,trainingcarNum,coachNum, lastPhotoNum);
