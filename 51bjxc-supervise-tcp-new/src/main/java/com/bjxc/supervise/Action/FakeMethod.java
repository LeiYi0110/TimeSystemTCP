...
public class FakeMethod {
	...
	public static void saveStudyTime(StudyingTime studyingTime,int trainingRecordId) {
		...
		trainingLog.setLocationid(locationInfo.getId());
...
//		}
	...
	public static Integer addTrainingRecord(String studentNum,int studentId,int coachId,int trainingCarId,int deviceId
			,int subjectId,int trainingLogCode,Date startTime,Date endTime,String deviceNum,String trainingcarNum,String coachNum,String lastImageNum) {
		...
		DeviceImage savePhoto1 = savePhoto(deviceNum, studentNum, imageNum1, 0);
		DeviceImage savePhoto2 = savePhoto(deviceNum, studentNum, imageNum2, 1);
		...
		trainingRecord.setStudentNum(studentNum);
		trainingRecord.setTrainingcarNum(trainingcarNum);
		trainingRecord.setDeviceNum(deviceNum);
		trainingRecord.setCoachNum(coachNum);
...
//		int avevelocity = (int) (mileage/((endTime.getTime()-startTime.getTime())/1000/60/60));
		...
		trainingRecord.setEtrainingNum(formatTrainingLogCode);
...
//		}
	 ...
	 * @return
	 */
	public static DeviceImage savePhoto(String deviceNum,String studentNum,String imageNum,int type) {
		DeviceImage deviceImage = new DeviceImage();
		deviceImage.setDeviceNum(deviceNum);
		deviceImage.setObjectNum(studentNum);
		deviceImage.setImageNum(imageNum);
		deviceImage.setUploadMode(1);	//1、自动上传
		deviceImage.setChannelNo(1);	//0、自动分配
		deviceImage.setImageSize((byte) 1);
		deviceImage.setEvent(type+17);	//type为0是学员登录，1是学员登出
		deviceImage.setCourseId(100);
		deviceImage.setLocationId(202);
		deviceImage.setFaceProbability(0);
		deviceImage.setPackageNum(0);
		deviceImage.setDataSize(0);
		deviceImage.setRecordTime(new Date());
